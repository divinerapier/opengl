...
static ShaderSources LoadShader(std::string filename) {
  ...
  while (getline(input, line)) {
    if (line.starts_with("#shader")) {
      ...
      } else if (line.ends_with("fragment") != std::string::npos) {
...
static unsigned int ComplieShader(unsigned int type,
  ...
  // specified by string. 替换 Shader 中的代码
  //
  // id: Shader 对象的描述符。
  //
  // count: string 数组的长度
  //
  // string: 代码数组，类型为 const char**，因此，可以传入多个 Shader
  // 代码，数量为 count
  //
  // length: 长度数组，类型为 const int*，数组中的每个元素表示对应 string
  // 参数元素的字符串长度，NULL 表示字符串以 NULL 结束 If length is NULL, each
  ...
  if (GL_FALSE == result) {
    ...
    std::cout << "Failed compile "
              ...
              << " shader: " << source << " error: " << message << std::endl;
    glDeleteShader(id);
    return 0;
  }
...
int main(void) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * sizeof(unsigned int), indexes,
               GL_STATIC_DRAW);

  ShaderSources sources = LoadShader("./resources/shaders/basic.glsl");
  unsigned int program =
      CreateProgram(sources.vertex_shader, sources.fragment_shader);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glDrawElements(GL_TRIANGLES, 6 /*indexes 的数量*/, GL_UNSIGNED_INT,
                   nullptr);
